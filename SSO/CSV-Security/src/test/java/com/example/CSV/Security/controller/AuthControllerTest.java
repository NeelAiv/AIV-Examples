...
class AuthControllerTest {
    ...
    void setUp() throws Exception {
        ...
        mockedHeaderSecurity = mockConstruction(HeaderSecurity.class, (mock, context) -> {
            when(mock.getSecure(any(Map.class), anyString(), anyString())).thenReturn("mocked-hex-payload");
        });
    ...
    void handleLogin_success() throws Exception {
        ...
        assertTrue(result.startsWith("redirect:http://localhost:9222/aiv/Default/sso_login?e="));
    ...
    @Test
    @DisplayName("authenticateApi should return 200 OK with secure payload on success")
    void authenticateApi_success() throws Exception {

        Map<String, Object> credentials = Map.of("deptCode", "Default", "userName", "Admin", "password", "password");
        Map<String, Object> userDetails = new HashMap<>();
        userDetails.put("userName", "Admin");

        when(csvAuthentication.authenticate(credentials)).thenReturn(userDetails);
        when(jwtTokenUtil.generateToken(anyString(), anyString())).thenReturn("dummy-token");

        ResponseEntity<String> response = authController.authenticateApi(credentials);

        assertEquals(200, response.getStatusCodeValue());
        assertEquals("mocked-hex-payload", response.getBody());
    }

    @Test
    @DisplayName("authenticateApi should return 401 Unauthorized on failure")
    void authenticateApi_failure() throws Exception {
        Map<String, Object> credentials = Map.of("deptCode", "Default", "userName", "Admin", "password", "wrongpassword");
        when(csvAuthentication.authenticate(credentials)).thenReturn(null);
